...
for case_study=1:3 %measurement zone radii cases
    ...
    for i_M = 1:M_run
        %initialization
        x_true{i_M}(:,1,case_study)= [0 0 0.01]'; %initial true robot states
        x_hat{i_M}(:,1,case_study)=[0,0,0.01]'; %initial predicted robot states 
        P_hat{i_M}(:,:,1)=eye(3,3)*2; %initial predicted robot error covariance
        Time(1)=0;
        j=1;
        %noise statistics
        sigma_Q=diag([0.4*delta_t 0.4*delta_t 0.4*delta_t]);  %standard deviation of measurement error
        Q=sigma_Q*sigma_Q; %process error covariance matrix
        ...
        for k=1:k_f
            ...
            for i_l = 1:35
                ...
                if range_to_landmark_i<=m_r
                    ...
                    %true measurements
                    range_to_landmark=sqrt((x_true{i_M}(1,k+1)-Xl{i_l}(1)).^2+(x_true{i_M}(2,k+1)-Xl{i_l}(2)).^2); %true range measurement 
                    bearing_to_landmark=atan2(x_true{i_M}(2,k+1)-Xl{i_l}(2),x_true{i_M}(1,k+1)-Xl{i_l}(1))-x_true{i_M}(3,k+1); %true bearing measurement
                    z_current_landmark=[range_to_landmark;bearing_to_landmark]+v(:,k+1); %true measurement vector at current landmark
                    %predicted measurements
                    range_pred=sqrt((x_bar{i_M}(1,k+1)-Xl{i_l}(1)).^2+(x_bar{i_M}(2,k+1)-Xl{i_l}(2)).^2); %predicted range measurement
                    bearing_pred=atan2(x_bar{i_M}(2,k+1)-Xl{i_l}(2),x_bar{i_M}(1,k+1)-Xl{i_l}(1))-x_bar{i_M}(3,k+1); %predicted bearing measurement    
                    ...
                    a = x_bar{i_M}(1,k+1)-Xl{i_l}(1);
                    b = x_bar{i_M}(2,k+1)-Xl{i_l}(2);
                    H_current=[((a)/(sqrt(a.^2+b.^2))),  ((b)/(sqrt(a.^2+b.^2))),  0; %linearized robot measurment matrix at current landmark
...
if study==1
    for case_study=1:3
        ...
        pbaspect([1 1 1]) %plot aspect ratio
        set(findobj(gcf,'type','axes'),'FontName','Arial','FontSize',12,'FontWeight','Bold', 'LineWidth', 2)
        ...
        grid on
        hold on
        title('Beacon-Based Localization: Robot Trajectories')
        ...
        yticks([1:nl])
        grid on
        title('Beacon-Based Localization: Detected Landmarks')
